...
p.st([sum(k*x[i+bi, j+bj, k] for i,j in rtB for k in N)==s
...
#in each col/row, only one number btw [i*n+1,(i+1)*n]
#note: this additional constriant is to reduce complexity
p.st([sum(x[r,c,k] for r in range(1,n+1) 
	for k in range(h*n+1, (h+1)*n+1))==1
	for h in range(n) for c in range(1,n+1)], 'row1')

p.st([sum(x[r,c,k] for c in range(1,n+1) 
	for k in range(h*n+1, (h+1)*n+1))==1
	for h in range(n) for r in range(1,n+1)], 'col1')

p.solver('intopt', 
        #this branching option usually helps a lot
        br_tech=pymprog.glpk.GLP_BR_PCH, #branching technique 
)
p.solve()
